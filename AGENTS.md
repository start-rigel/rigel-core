...
## 当前主链路
1. `rigel-jd-collector` 调用京东联盟接口并写入原始商品与价格
2. `rigel-build-engine` 将原始商品整理为型号级价格清单
3. `rigel-build-engine` 组装 `用户需求 + 价格清单` 并请求 AI API
4. `rigel-console` 展示结构化推荐结果

## 模块职责
- `rigel-jd-collector`: 京东联盟查询与原始数据入库
- `rigel-build-engine`: 接收界面参数、整理硬件信息、构建 AI 请求、返回分析结果
- `rigel-console`: 最小前端与 API 入口
